...
def draw_annotation_bbox_mask(img, boxes, klass, seg_mask):
    """Will not modify img"""
    labels = []
    ...
    img = draw_GTmask(img, seg_mask)  # draw GT_seg mask
    return img


def draw_annBox_and_predBox(img, boxes, klass, results, is_crowd=None):
    """Will not modify img"""
    labels = []
    assert len(boxes) == len(klass)
    ...
    img = viz.draw_boxes(img, boxes, tags)
...
def draw_final_outputs(img, results, seg_mask):
    """
    Args:
    ...
    ret = draw_GTmask(ret, seg_mask)  # draw GT_seg mask
...
def draw_mask(im, mask, alpha=0.5, color=None):
    ...
    if color is None:
        ...
        color = np.array([0,0,255]).astype(np.float32)[::-1]
...
def draw_GTmask(im, mask, alpha=0.5, color=None):
    """
    Overlay a mask on top of the image.
    ...
    if color is None:
        # color = PALETTE_RGB[np.random.choice(len(PALETTE_RGB))][::-1]
        color = np.array([255,255,0]).astype(np.float32)[::-1]

    # print(mask[0].shape, im.shape)

    im = np.where(np.repeat((mask[0] > 0)[:, :, None], 3, axis=2),
