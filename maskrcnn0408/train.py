...
from data import get_all_anchors, get_all_anchors_fpn, get_eval_dataflow, get_train_dataflow, get_eval_dataflow_for_visual
from eval import DetectionResult, predict_image, multithread_predict_dataflow, EvalCallback, predict_image_for_viz
...
from viz import draw_annotation, draw_final_outputs, draw_predictions, draw_proposal_recall, draw_annBox_and_predBox
...
def do_visualize(model, model_path, data_test, output_dir='output'):
    ...
    predcfg = OfflinePredictor(PredictConfig(
                model=model,
                session_init=get_model_loader(model_path),
                input_names=model.get_inference_tensor_names()[0],
                # input_names=model.get_inference_tensor_names()[0] + ['gt_boxes', 'gt_labels'],
                output_names=[
                    'generate_{}_proposals/boxes'.format('fpn' if cfg.MODE_FPN else 'rpn'),
                    'generate_{}_proposals/scores'.format('fpn' if cfg.MODE_FPN else 'rpn'),
                    'fastrcnn_all_scores'] + model.get_inference_tensor_names()[1]))
    if os.path.isdir(output_dir):
        shutil.rmtree(output_dir)
    utils.fs.mkdir_p(output_dir)
    ...
    with tqdm.tqdm(total=nr_visualize) as pbar:
        for idx, dp in itertools.islice(enumerate(df), nr_visualize):
            img, gt_boxes, gt_labels, filename, gt_masks = dp['image'], dp['gt_boxes'], dp['gt_labels'], dp['fname'], dp['gt_masks']
            fname = filename.split('/')[-1][:-4]
            print('before:', fname, idx)

            rpn_boxes, rpn_scores, all_scores, results = predict_image_for_viz(img, predcfg)

            # draw groundtruth boxes
            gt_viz = draw_annBox_and_predBox(img, gt_boxes, gt_labels, results)
            # draw best proposals for each groundtruth, to show recall
            proposal_viz, good_proposals_ind = draw_proposal_recall(img, rpn_boxes, rpn_scores, gt_boxes)
            # draw the scores for the above proposals
            score_viz = draw_predictions(img, rpn_boxes[good_proposals_ind], all_scores[good_proposals_ind])
            final_viz = draw_final_outputs(img, results, gt_masks)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_data', help="Run evaluation. ")
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        for i in range(6, 10):
            ...
            MODEL = ResNetFPNModel() if cfg.MODE_FPN else ResNetC4Model()
            DetectionDataset() 
            each_logdir = args.logdir + 'f_' + str(i)
            ...
            if not is_horovod or hvd.rank() == 0:
                logger.set_logger_dir(each_logdir, 'd')
            ...
            callbacks = [
                ...
                #     every_k_epochs=10),
                ModelSaver(max_to_keep=3, keep_checkpoint_every_n_hours=1),
                # linear warmup
                ...
                EvalCallback(val_name, *MODEL.get_inference_tensor_names(), each_logdir),
                MaxSaver(monitor_stat='mAP(bbox)/IoU=0.5:0.95', filename='mAP')
            ] 
            if not is_horovod:
            ...
            train_dataflow = get_train_dataflow(str(i))

            print('len(train_dataflow)', len(train_dataflow))
