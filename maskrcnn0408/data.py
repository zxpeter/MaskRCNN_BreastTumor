...
from tensorpack.dataflow import (
    DataFromList, MapDataComponent, MultiProcessMapDataZMQ, RepeatedDataPoint, MapDataComponent, MapData, MultiThreadMapData, TestDataSpeed, imgaug)
from tensorpack.utils import logger
from tensorpack.utils.argtools import log_once, memoized

from common import (
    CustomResize, DataFromListOfDict, box_to_point8,
    filter_boxes_inside_shape, point8_to_box, segmentation_to_mask, np_iou, ElasticDeform)
...
def get_train_dataflow(i):
    ...
    def preprocess(roidb):
        ...
        # viz = draw_annotation_bbox_mask(im, boxes, klass, masks)
        # cv2.imwrite("{}_{}.png".format(fname[-7:-4], roidb['aug_name']), viz)
        return ret
...
def get_eval_dataflow_for_visual():
    ...
    def preprocess(roidb):
        ...
        if cfg.MODE_MASK:
            ret['gt_masks'] = roidb['segmentation']
    ...
    if cfg.TRAINER == 'horovod':
        ...
        ds = MultiProcessMapDataZMQ(ds, 10, preprocess, buffer_size =50, strict=True)
    # ds = MapDataComponent(ds, preprocess, 0)
    return ds
